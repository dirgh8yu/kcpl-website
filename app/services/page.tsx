...
export default function ServicesPage() {
  return <><PageIntro label="Services / Air · Ocean · Road" title={<>The right movement.<br />The right coordination.</>}><p>International freight and Nepal-side execution, connected through a clearly defined shipment scope.</p></PageIntro>
    <Container><nav className="service-jump" aria-label="Services on this page">{services.map(s => <a href={`#${s.id}`} key={s.id}>{s.title}</a>)}</nav><ResponsiveImage image="ocean" className="wide-photo" caption="Maritime freight context. Illustrative stock." /></Container>
    <section className="section"><Container>{services.map(s => <article className="service-detail" id={s.id} key={s.id}><div><Eyebrow>{s.number} / Service</Eyebrow><h2>{s.title}</h2>{s.id === 'project-cargo' && <TextLink href="/project-cargo">Our project cargo capability</TextLink>}{s.id === 'warehousing' && <TextLink href="/network#warehouse-facilities">View facility locations</TextLink>}</div><div><p className="service-lead">{s.detail}</p><ul className="plain-list">{s.scope.map(item => <li key={item}>{item}</li>)}</ul><details className="planning-disclosure"><summary>Before you enquire</summary><p>{s.note}</p></details><TextLink href={`/contact?service=${s.id}`}>Enquire about {s.title.toLowerCase()}</TextLink></div></article>)}</Container></section><QuoteCTA /></>;
}
