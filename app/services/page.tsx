...
export default function ServicesPage() {
  return <><PageIntro label="Services / Air · Ocean · Road" title={<>The right movement.<br />The right coordination.</>}><p>International freight and Nepal-side execution, connected through a clearly defined shipment scope.</p></PageIntro>
    <Container><nav className="service-jump" aria-label="Services on this page">{services.map(s => <a href={`#${s.id}`} key={s.id}>{s.title}</a>)}</nav><ResponsiveImage image="ocean" className="wide-photo" caption="Maritime freight infrastructure." /></Container>
