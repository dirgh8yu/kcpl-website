...
export default async function ContactPage({ searchParams }: { searchParams: Promise<{ service?: string }> }) {
  ...
  return <><div className="contact-intro"><PageIntro label="Contact / Request a quote" title={<>Let’s start with the cargo.</>}><p>Tell us what needs to move and where it needs to go. We’ll discuss the freight, customs and delivery scope.</p></PageIntro></div>
    <section className="contact-section"><Container>
      ...
      <div className="contact-grid"><div className="contact-form-column">{!isPublicSite && <p className="review-notice">Private review — use demonstration details and files only.</p>}<EnquiryForm initialService={initialService} /><noscript><p>Please send your enquiry directly to <a href={`mailto:${company.email}`}>{company.email}</a>. The online enquiry form requires JavaScript.</p></noscript></div>
